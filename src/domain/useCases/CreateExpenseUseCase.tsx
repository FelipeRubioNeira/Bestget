...
class CreateExpenseUseCase {
    ...
    async create(
        ...
        if (!isValid) return this.throwMessage(message)

        const expenseCreated = await this.expenseRepository.create(expense)

        if (!expenseCreated) return this.throwMessage("No se pudo guardar el gasto.")

        validationResult.result = expenseCreated

        const eventName = expense.budgetId ? EventNames.EXPENSE_CREATED_FROM_BUDGET : EventNames.EXPENSE_CREATED
